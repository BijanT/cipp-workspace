#!/bin/bash
timestamp="output_colloid_bfs_$(date +"%m%d%Y_%H%M")"
...
## output files
bfs_dir="${current_dir}/${timestamp}/bfs"
...
mkdir -p $bfs_dir $bwmon_dir $latency_dir $vmstat_dir
...
for current_setting in "${local_remote[@]}"; do
        ...
        for current_core in "${cpu_core_list[@]}"; do
                for trial in $(seq 0 2); do
                        ...
                        bfs_out_file=${bfs_dir}/${bfs_file}_trial_${trial}_cpu_${current_core}_${current_setting}.log
                        ...
                        if [ "$current_setting" = "local" ]; then
                                ...
                                numactl -m 0 taskset -c 0-$((current_core - 1)) $bfs_exe -g 30 > ${bfs_out_file}&

                        else

                                taskset -c 0-$((current_core - 1)) $bfs_exe -g 30 > ${bfs_out_file}&

                        fi

                        bfs_pid=$!
 
                        taskset -c $rsvd_core $bwmon_exe $bwmon_sample_rate "${bwmon_out_file}" $bfs_pid &
                        ...
                        bfs_avg_time=$(cat "${bfs_out_file}" | grep "Average Time" | grep -oP '\d+\.\d+')
                        ...
                        printf "$tabular_data_print" "$current_setting" "$current_core" "$trial" $bfs_avg_time $avg_bw $avg_latency
                done
        done
