...
for current_setting in "${local_remote[@]}"; do
        ...
        for current_core in "${cpu_core_list[@]}"; do
                for trial in $(seq 0 2); do
                        ...
                        clover_out_file=${clover_dir}/${clover_file}_trial_${trial}_cpu_${current_core}_${current_setting}.log
                        ...
                        taskset -c $rsvd_core $bwmon_exe $bwmon_sample_rate "${bwmon_out_file}" $clover_pid &
                        ...
                        clover_avg_time=$(cat "${clover_out_file}" | grep "Wall clock" | tail -n1 | grep -oP '\d+\.\d+')
                        ...
                        printf "$tabular_data_print" "$current_setting" "$current_core" "$trial" $clover_avg_time $avg_bw $avg_latency
                done
        done
